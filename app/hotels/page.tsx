        ...
        {loading ? (
          ...
          <div className="space-y-10">
            {filtered.map((hotel, index) => (
              <div
                key={hotel._id}
                className="bg-white rounded-3xl overflow-hidden shadow-sm hover:shadow-xl border border-stone-200 transition-all"
              >
                <div
                  className={`grid grid-cols-1 lg:grid-cols-2 ${
                    index % 2 ? 'lg:[&>*:first-child]:order-last' : ''
                  }`}
                >
                  ...
                  <div className="p-10 flex flex-col justify-center">
                    ...
                    <div className="flex flex-wrap gap-2 mb-6">
                      {hotel.amenities.slice(0, 6).map(amenity => (
                        <span
                          key={amenity}
                          className="text-xs bg-amber-50 text-amber-700 border border-amber-100 px-3 py-1.5 rounded-full font-semibold"
                        >
                          {amenity}
                        </span>
                      ))}
                    ...
                    <div className="flex items-center justify-between pt-5 border-t border-stone-100">
                      ...
                      <Link
                        ...
                        className="flex items-center gap-2 bg-amber-500 hover:bg-amber-600 text-white px-5 py-2.5 rounded-xl text-sm font-bold transition-all shadow-md"
                      >
